...
# ---------------------------------------------------------------------------
# Paragraph snapshot store — persists across tool calls within a server session.
# Keyed by normalised document name (lowercase) → {timestamp, paragraphs}.
# ---------------------------------------------------------------------------
...
async def word_live_get_diff(filename: str = None) -> str:
    ...
    (created automatically by word_live_get_text or word_live_take_snapshot).
    Returns added, modified, and deleted paragraphs with their indices.

    If no snapshot exists, returns an error prompting a full read first.
        ...
        if snap is None:
            return json.dumps({
                "error": "No snapshot exists for this document. "
                         "Call word_live_get_text or word_live_take_snapshot first.",
            })

        old_paras = snap["paragraphs"]          # list[{index, text}]
    ...
    try:
        ...
        return json.dumps({
            ...
            "snapshot_timestamp": snap["timestamp"]
                if (snap := _get_snapshot(doc)) else None,
...
async def word_live_get_text(filename: str = None) -> str:
    ...
    try:
        ...
        paragraphs = _read_paragraphs(doc)

        # Auto-store snapshot for later diffing
        _store_snapshot(doc, paragraphs)
